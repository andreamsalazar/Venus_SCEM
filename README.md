...
This repository contains the code and data needed to run the Venus Spin–Climate Evolution Model (Venus-SCEM) and reproduce the principal results and figures from:
...
- **`concatenate_sorting.py`**  
  Merges the batch outputs into a single `.pkl` file and summarizes the final-state distribution. This script is called automatically after all batch jobs finish successfully. After merging, it removes the individual batch directories.
...
- **`all_solutions.pkl`**  
  Merged output from the baseline ensemble of 50,000 simulations. These results are used to produce Figures 2–4 of the main text.
...
- **`Figure_2.ipynb`**  
  Reproduces Figure 2 of the main text.

- **`Figure_3.ipynb`**  
  Reproduces Figure 3 of the main text.

- **`Figure_4.ipynb`**  
  Reproduces Figure 4 of the main text.

## Sensitivity tests

### `sensitivity_tests/`

Each directory below contains an `all_solutions.pkl` file and the corresponding `venus_run_input` parameter file. Some directories also contain a modified version of `run_Venus_evolution.py` when changes to the model source code were required.

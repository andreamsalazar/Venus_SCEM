...
T_transition = float(sys.argv[23]) # set transition from rapid to slow rotation [days]

omega_transition = 2*np.pi/(T_transition*60*60*24)
...
Q_data = np.load('/n/wordsworth_lab/asalazar/Venus_Evolution/Q_withIT_330.npy')
sig_data = np.load('/n/wordsworth_lab/asalazar/Venus_Evolution/sig_330_Green.npy')
...
def spin_evolution_hab(t,x, rheo_model,tau_lw_habitable, tau_sw_habitable, p_s_habitable, delta_t_s, Q, Qn):
    ...
    #albedo = float(interp_albedo([sig_albedo, S])) + 0.05 # interpolate Yang 2014 50-m data
    #rot_period = abs(2*np.pi/(pad_omega*60*60*24))
    albedo = 0.3+0.5*0.4*(1+np.tanh((omega_transition - abs(pad_omega))/n)) # transition from rap to slow pre-set
...
def run_Venus_evolution(delta_t_s, L_o, obl_o, tau_lw, tau_sw, ps, Q, Qn,Q_ocean,tau_lw_Venus, tau_sw_Venus,tau_sw_steam, ramp_rate): 
    ...
    rot_period_endsteam = abs(2*np.pi/(omega_steam[-1]*60*60*24))
    albedo_endsteam = 0.3+0.5*0.4*(1+np.tanh(4*(rot_period_endsteam-T_transition)/T_transition))

...
Q_data = np.load('/n/wordsworth_lab/asalazar/Venus_Evolution/Q_withIT_330_mycalc.npy')
...
def spin_evolution_steam(t,x, rheo_model, tau_sw_steam):
    ...
    if rheo_model == 'Ocean_Andrade':
        def b_g_sig(sigma):
            k2Q_ocean = 0.2/f_interp_Qocean(sigma) * np.tanh((sigma)/(0.1*n)) # smooth change of sign  
